...
# 3.5. Функція створення GitHub Issue
def create_github_issue(title, body, label, token, repo):
            ...
            if st.button("Класифікувати звернення"):
                if user_input.strip():
                    with st.spinner('🤖 Аналізую текст...'):
                        ...
                        if st.button("🚀 Створити Issue в GitHub"):
                            gh_token = config.get('github', {}).get('token', 'YOUR_GITHUB_TOKEN_HERE')
                            gh_repo = config.get('github', {}).get('repo', 'owner/repo_name')
                            if gh_token == "YOUR_GITHUB_TOKEN_HERE" or not gh_token:
                                st.warning("⚠️ Будь ласка, додайте свій справжній GitHub Token у файл config.yaml")
                            else:
                                title = user_input[:50] + ("..." if len(user_input) > 50 else "")
                                success = create_github_issue(title, user_input, prediction, gh_token, gh_repo)
                                if success:
                                    st.success("✅ Задачу успішно відправлено розробникам у GitHub!")
                                else:
                                    st.error("❌ Помилка при створенні задачі. Перевірте токен або назву репозиторію.")
